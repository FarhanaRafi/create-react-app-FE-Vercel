...
const Blog = (props) => {
  ...
  const handleImageUpload = async (id) => {
    try {
      ...
      let response = await fetch(`${url}/blogPosts/${id}/uploadCover`, {
        method: "POST",
        body: formData,
      });
      if (response.ok) {
  ...
  const fetchBlog = async (id) => {
    let res = await fetch(url + "/blogPosts/" + id);
    if (res.ok) {
  ...
  const getBlogComments = async (id) => {
    try {
      let response = await fetch(`${url}/blogPosts/${id}/comments`);
      if (response.ok) {
  ...
  const handleDelete = async (postId, commentId) => {
    try {
      await fetch(`${url}/blogPosts/${postId}/comments/${commentId}`, {
        method: "DELETE",
      });
    } catch (error) {
      console.log(error);
